...
with tab1:
    ...
    for col, label in GROUP_COLS:
        ...
        # JavaScript (all JS curly braces escaped)
        d3_code = f"""
...
with tab2:
    ...
    if st_autorefresh:
        st_autorefresh(interval=REFRESH * 1000, key="family_autorefresh")
    ...
    def fetch_realtime_blocks(n_sales: int):
        ...
        saleid_place = ','.join(['%s'] * len(saleids))

        # 2) sales items
        salesitems = db.fetch_data(
            f"SELECT saleid, itemid, quantity, totalprice "
            f"FROM salesitems WHERE saleid IN ({saleid_place})",
            saleids
        )
        if salesitems.empty:
        ...
        items = db.fetch_data(
            f"""
            SELECT itemid, familycat, sectioncat,
    ...
    if top_groups.empty:
        ...
        rt_data  = [{"group": str(r[sel_col]), "total_sales": float(r["totalprice"])}
                    for _, r in top_groups.iterrows()]
        rt_json  = json.dumps(rt_data)
